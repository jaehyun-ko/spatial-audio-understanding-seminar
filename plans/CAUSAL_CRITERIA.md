...
관찰 기준은 좌표 계산을 설명한 뒤에 제시한다. S07–S13 GCC-PHAT·SRP·Neural-SRP → S14 좌표만으로 부족한 질문 → S15 E/P/LLM·LoRA → S16 성질의 기준 순서다. S41은 H/Z/답의 서로 다른 관측을 설명하고 S47은 같은 장면으로 검증할 미실행 실험을 제안한다.

이전 자산 자체 검수 기록은 `plans/archive/pre-localization/plans/CAUSAL_CRITERIA.md`에 보존했다. 이번 조립본과 내보내기 검수는 `output/qa/localization/` 및 `output/manifest.json`을 따른다. 앞선 자산 검수만으로 현재 덱 전체를 검수했다고 말하지 않는다.
