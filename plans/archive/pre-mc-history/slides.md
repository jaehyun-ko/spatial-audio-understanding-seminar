...
description: 'GCC-PHAT 시간차 추정과 SRP 좌표 탐색, Neural-SRP 응답 학습을 설명한 뒤 LLM 통합으로 연결한다: 본문 47장과 근거 부록 33장'
aspectRatio: 16/9
canvasWidth: 1280
colorSchema: light
transition: none
...
::source::
본문 47장 · 근거 부록 33장 · 27편의 설계와 검증 범위
...
[설명 순서]
앞에서 SRP와 Neural-SRP로 후보 공간의 최대 응답 위치를 골랐다. 그러나 그 좌표만으로는 어떤 소리였는지, 언제 났는지 답할 수 없다. 여기부터는 위치 추정기의 출력을 넘어 여러 질문에 필요한 정보를 표현에 남기고 LLM에 전달하는 설계 문제를 다룬다. Neural-SRP 논문이 자신의 내부 특징의 범용성이나 LLM 연결을 입증했다는 뜻은 아니다. 다음 장에서 인코더 E, 어댑터 P, LLM 적응의 전체 구조를 먼저 본다.
...
::body::
<PaperFigure src="/diagrams/causal-audio-llm-contract.svg" alt="다채널 파형 X가 인코더 E를 거쳐 T×de 표현 H가 되고 어댑터 P에서 K×dLLM 연속 오디오 토큰 Z가 된다. 질문 임베딩과 함께 LoRA 등으로 적응한 LLM에 입력되어 답변을 생성한다." />

::takeaway::
**인코더의 표현을 어댑터로 연결**하고, **LoRA 등으로 LLM을 적응**한다.
...
[연결]
다음에 확인할 질문: 사전학습 목표와 표현의 재사용 시험을 분리한다
...
[연결]
다음에 확인할 질문: 공간 단서를 목표로 배운 초기화는 잡음에서 시험한다
...
[연결]
다음에 확인할 질문: “뒤쪽의 알람”은 종류와 위치가 함께 맞아야 한다
...
layout: "seminar"
variant: "figure"
chapter: "03 · 그 정보를 어떤 토큰으로 전달할 것인가"
causalStage: "main"
originSlide: 32
---
...
[연결]
다음에 확인할 질문: 의미와 공간을 따로 배우고 같은 음원에 연결한다
...
[연결]
다음에 확인할 질문: 의미 정렬을 더하면 검색과 위치 판독이 어떻게 바뀌나
...
[연결]
다음에 확인할 질문: 기존 의미 경로에 공간 토큰 경로를 더한다
...
[연결]
다음에 확인할 질문: 시간별 궤적을 남긴 토큰을 LLM에 전달한다
...
[연결]
다음에 확인할 질문: 같은 토큰 수에서도 시간 구조를 배운 표현은 다르다
...
[연결]
다음에 확인할 질문: 음원 단위 감독과 LLM에 보내는 특징을 구별한다
...
[연결]
다음에 확인할 질문: 인코더가 만든 특징의 의미와 LLM의 입력 해석을 연결하려면 어떤 모듈을 언제 고정하고 학습할지 정해야 한다.
...
[설명의 중심]
인코더가 만든 특징의 의미와 LLM의 입력 해석을 연결하려면 어떤 모듈을 언제 고정하고 학습할지 정해야 한다.

[연결]
다음에 확인할 질문: 표현 정렬·언어 적응·공간 경로 학습을 단계로 나눈다
...
[연결]
다음에 확인할 질문: 공간 토큰을 쓰는 모델과 zero-spatial 변형을 비교한다
...
# 공간 토큰을 쓰는 모델과 zero-spatial 변형을 비교한다
...
- https://arxiv.org/html/2606.10738v2
[발표 노트]
S32에서 구분한 세 질문으로 돌아간다. SO-7B-zs는 공간 토큰을 0으로 만든 조건이고 SO-7B는 실제 공간 토큰을 받는다. EAzi는 방위각 오차가 20도 안에 들어오는 비율, IS-Loc은 위치 기반 식별, RLR은 상대 좌우 판정이다. 선택한 세 점수가 실제 공간 토큰 조건에서 모두 높다. 같은 7B 계열의 내부 비교로 읽는다.
[해석 범위]
...
[연결]
다음에 확인할 질문: 언어 학습 목표를 바꾸면 지각과 복합 질의가 함께 바뀐다
...
LoRA 없이 학습 연결 모듈과 frozen LLM을 사용하는 BLIP-2 같은 설계도 있다. 이는 별도 모달리티의 구조적 반례이며 공간 오디오에서 동일 성능을 보장한다는 주장으로 확대하지 않는다.
[해석 범위]
...
# 언어 학습 목표를 바꾸면 지각과 복합 질의가 함께 바뀐다
...
<p class="seminar-chart-note">동일 checkpoint·학습 예산의 완전 통제는 미명시 · 앞의 exact-MCQA 점수와 채점법이 다름</p>
</div>
...
[연결]
다음에 확인할 질문: 인코더가 같은 관측에서 과제별 특징을 고르게 한다
...
이 표의 semantic-answer judge는 질문·선지·gold·생성 답변을 받는 Gemini 3 Flash이며 audio/RTSD를 직접 보지 않는다. 최종 SAPO 이전의 진단이다. 앞의 main Table 5에서 SFT/SAPO를 비교한 exact MCQA와 채점법이 다르므로 숫자의 높낮이를 두 표 사이에서 비교하지 않는다.

원문은 projector ablation이라고 명명하지만 각 행의 encoder checkpoint, LLM checkpoint, 학습 예산과 seed가 완전히 같은지 별도 명시하지 않는다. 따라서 연결 설계별 보고 결과로 해석한다. P만 바꾸고 나머지를 완벽히 고정한 인과 실험이라고 강화하지 않는다. 이어지는 DSpAST는 같은 QA 인터페이스의 인코더 비교, OWL은 인코더 사전학습 loss 비교로 읽는다.
...
[연결]
다음에 확인할 질문: LLM 인터페이스를 맞추고 인코더의 차이를 비교한다
...
[연결]
다음에 확인할 질문: 기하 지도는 인코더에 배우고, QA에서는 오디오만 읽는다
...
[해석 범위]
이 비교에서는 인코더 특징뿐 아니라 사전학습 curriculum과 AdaCos loss도 다르다. 차이 전체를 세 분기에 귀속할 수 없다. 원 BAT와 checkpoint 및 의미 평가 embedding이 달라 S76 수치와 직접 연결하지 않는다. 원문의 질문 유형 설명 일부 오기는 BAT의 유형 정의를 따른다.
...
[연결]
다음에 확인할 질문: 기하 loss의 효과와 LLM 적응 효과는 다른 실험이다
...
# 기하 loss의 효과와 LLM 적응 효과는 다른 실험이다
...
[연결]
다음에 확인할 질문: 관계 QA의 개선이 목표 발화 전사를 보장하지는 않는다
...
# 관계 QA의 개선이 목표 발화 전사를 보장하지는 않는다
...
layout: "seminar"
variant: "figure"
chapter: "06 · 답이 실제 소리에 근거하는지 확인하기"
causalStage: "main"
---
...
[발표 노트]
S43의 두 거리를 회수하고 양수/음수의 뜻을 설명한다. 선택 모델들의 반응을 같은 자극에서 읽되 내부 전처리는 다름을 표시한다. 왼쪽 채널만 쓰는 mono 대조군은 입력이 변하지 않아 구성상0이라는 점을 말한다.
...
[연결]
다음에 확인할 질문: 각 논문을 E의 정보 보존, P의 구조 전달, LLM의 입력 사용 중 어디를 확인했는지로 정리한다.
...
[설명의 중심]
각 논문을 E의 정보 보존, P의 구조 전달, LLM의 입력 사용 중 어디를 확인했는지로 정리한다.
...
::bridge::
A 관측과 인코더 · B 의미와 공간의 결속<br/>
C 토큰으로 보내는 관측 · D 시간 정보의 전달 · E 답변의 근거
...
[설명의 중심]
본문은 47장에서 마쳤으며 이후는 질문에 따라 선택해 보는 상세 근거다.
...
[상세 근거와 해석 범위]
[근거 부록의 사용]
본문은 앞의 47장에서 마쳤다. 이 부록은 논문을 순서대로 추가 발표하기 위한 두 번째 본문이 아니다. 질문이 나오면 해당 설계 경계나 검증 조건으로 이동해 필요한 그림·표를 확인한다. 다섯 구역은 관측과 인코더, 의미와 공간 결속, 토큰으로 보내는 관측, 시간 정보 전달, 답변의 근거다. 장 번호와 논문별 연결은 CAUSAL_DECK_ORDER와 본문 근거 대장에 기록한다. 모든 수치는 기존 지정 원문의 조건에 한정한다. 설명용 범위·발표자 정리.
-->

---
layout: "seminar"
variant: "figure"
chapter: "근거 부록 A · 배열과 인코더"
causalStage: "appendix"
originSlide: 12
---
...
[연결]
이 장은 본문의 A · 배열과 인코더 질문을 보완하는 선택 자료다.
...
[연결]
이 장은 본문의 B · 의미와 공간의 결속 질문을 보완하는 선택 자료다.
...
[연결]
이 장은 본문의 C · 토큰으로 보내는 관측 질문을 보완하는 선택 자료다.
...
[해석 범위]
이후 제안할 채널별 대조·동일 parser·추가 pooling/판독 실험은 미실행이다. 이 결과는 배열 일반화나 모든 질문 응답 모델의 인과 추론을 시험하지 않는다. 상세 유의성 검정은 원문의 추가 자료이다.
