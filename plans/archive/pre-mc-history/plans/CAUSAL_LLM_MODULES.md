...
S15에서 위치 추정 뒤의 공통 E/P/LLM·LoRA 구조를 먼저 소개한다. 이후 S22–S29의 토큰 구조, S30–S34의 학습, S35–S40의 모듈 비교를 그 관점으로 읽는다. S15의 그림에 포함된 LoRA는 LLM 안에서 처리 방식을 바꾸며 연속 토큰의 생성 위치인 P와 구별한다.

이전 78장 통합 검수는 `plans/archive/pre-localization/plans/CAUSAL_LLM_MODULES.md`에 보존했다. 이번 80장 조립과 내보내기 검수 상태는 `output/qa/localization/` 및 `output/manifest.json`을 따른다.
