...
예측한 속성을 텍스트/JSON으로 전달하는 Motion 계열, 좌표 head만 필요한 위치 추정, text–audio retrieval도 범위 밖에 존재한다. 이들의 표준 파이프라인에 연속 audio token projector나 LLM LoRA가 반드시 포함되는 것으로 그리지 않는다. 기존 `EVIDENCE_P12_P23.md`의 입력·데이터·평가 한계를 유지한다.
...
## 6. 추가 bridge 슬라이드와 재현

- `slides/bridges/audio-llm-contract.md`: E/P/LLM 역할과 연속 임베딩 입력.
- `slides/bridges/token-bottleneck.md`: 단순 평균의 시간 결속 손실 반례, ST-AudioLM/TWNM 조건.
- `slides/bridges/adapter-training.md`: 흔한 학습 순서와 Spatial-Omni/Sci-Phi의 실제 차이.
- `slides/bridges/lora-reading.md`: P와 LoRA의 서로 다른 위치, 저랭크 수식, 기존 어휘 생성.

원본 그림은 직접 제작한 과학 교육용 벡터/래스터이며 실험 예측을 그린 것이 아니다. `scripts/visuals/causal-llm.py`가 `public/diagrams/causal-{audio-llm-contract,token-bottleneck,adapter-training,lora-reading}.{svg,png}`를 생성한다. 규격1136×404 SVG/2272×808 PNG. 라이트 디자인 시스템, Pretendard, matplotlib mathtext 경로 렌더. 실행: `.venv/bin/python scripts/visuals/causal-llm.py` (matplotlib/numpy 및 Pretendard 설치 필요). 글자 bounds와 실제 렌더 이미지 검수.
...
`.venv/bin/python -m py_compile scripts/visuals/causal-llm.py` 통과. 4개 최종SVG의 한글·수식은 경로로 저장했고 PNG는2272×808이다. 실제 PNG4개를 확대해 확인하고 겹치던 선 위 라벨을 이동했으며 LoRA 경로에 alpha/r와 후속 출력 헤드를 명시했다. 모든 텍스트 bounds가 canvas 내부임을 생성기가 확인한다.

통합 후 `node scripts/visual-qa.mjs --url http://localhost:3037 --slides 22,26,31,34 --out tmp/causal-bridge-qa --fail-on-warnings`가 4장 모두0 review candidates로 통과했다. 이어 본문23–25/27–30/32–33/35–41의16장도 `tmp/causal-middeck-qa`에 렌더해 모두0 review candidates. 전체20장의 본문·노트를 논리 검수했고, 원문 구조도6장은 확대 확인했다. 장26의 챕터 라벨은 별도 재캡처에서 y=18px, 정상 위치를 확인했다. S41의 targeted transcription 해석과 S23/S38의 용어 보완을 조립 담당에게 보고했다.
