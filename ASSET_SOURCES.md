# 현재 MC Audio-LM 연구사 개정 자산 — 2026-09-11

현재85장(본문49+부록36)의 순서와 번호는 [CAUSAL_DECK_ORDER.json](plans/CAUSAL_DECK_ORDER.json)을 따른다. 아래 이전 자산 목록의 장 번호는 제작 당시 번호이며 원문/생성 자산은 그대로 보존했다.

| 현재 장 | 자산 | 근거와 용도 |
| --- | --- | --- |
| S15 | history-map.svg/png | 최초 공개일과 연구 질문의 분화; arXiv 제출 이력 |
| S16 | history-interfaces.svg/png | Motion §2.2 JSON과 OWL §5 연속 토큰 |
| S22 | history-bat.svg/png | Pengi의 고정 LM 배경과 BAT v1 공간 QA 연결 |
| S24 | history-encoder-branches.svg/png | DSpAST 특징별 attention, OWL 기하 보조 감독 |
| S28 | history-extensions.svg/png | 기존 의미 경로와 새 공간 경로의 공통 분석 |
| S37 | history-dynamics.svg/png | Motion·Dynamic QA·ST-AudioLM 시간 표현 비교 |
| S48 | native HTML/PPTX table | 연구 질문·전달 경계·관찰 기준의 종합 |

생성 원본은 `scripts/visuals/mc-history-spine.py`, `scripts/visuals/mc-history-interfaces.py`이며 그림의 텍스트 경계도 보존한다. 교육용 재구성이고 실제 activation이나 실험 결과가 아니다. S48의 독립 그림 후보는 보존하지만 활성 덱은 편집 가능한 표를 쓴다. 원문 URL·섹션·채택 버전·실제 계승 여부는 [MC_AUDIO_LM_HISTORY.md](plans/MC_AUDIO_LM_HISTORY.md)와 각 발표 노트에 있다. S17 공통 구조의 LM 블록은 '고정 또는 적응'으로 수정해 연속 입력과 LoRA를 분리했다.
